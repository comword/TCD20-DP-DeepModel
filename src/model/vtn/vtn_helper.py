# https://github.com/bomri/SlowFast/blob/master/slowfast/models/vtn_helper.py
import torch
...
class VTNLongformerModel(LongformerModel):
    ...
    def __init__(self,
        ...
        super(VTNLongformerModel, self).__init__(
            self.config, add_pooling_layer=False)
        # to avoid distributed error of unused parameters
...
def pad_to_window_size_local(input_ids: torch.Tensor, attention_mask: torch.Tensor, position_ids: torch.Tensor,
    ...
    input_ids = F.pad(input_ids.permute(0, 2, 1),
                      (0, padding_len), value=pad_token_id).permute(0, 2, 1)

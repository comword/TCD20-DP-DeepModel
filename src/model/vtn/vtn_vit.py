...
class VTNVIT(tf.keras.Model):
    def __init__(self, cfg, **kwargs):
        super(VTNVIT, self).__init__(**kwargs)

        self.img_input = tf.keras.layers.Input(cfg.input_shape)
        self.pos_input = tf.keras.layers.Input(cfg.input_shape[1])
        assert cfg.input_shape[2]==cfg.input_shape[3]
        self.backbone = getattr(vit, cfg.backbone)(
            image_size=cfg.input_shape[2], activation='relu', pretrained=True, include_top=False, pretrained_top=False)

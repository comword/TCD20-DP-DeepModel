...
class TFLongformerEmbeddings(tf.keras.layers.Layer):
    ...
    def call(self, input_ids=None, position_ids=None, token_type_ids=None, inputs_embeds=None, training=False):
        ...
        if input_ids is not None:
            inputs_embeds = tf.gather_nd(
                params=self.weight, indices=tf.expand_dims(input_ids, -1))
        ...
        position_embeds = tf.gather_nd(
            params=self.position_embeddings, indices=tf.expand_dims(position_ids, -1))
        token_type_embeds = tf.gather_nd(
            params=self.token_type_embeddings, indices=tf.expand_dims(token_type_ids, -1))
...
class TFLongformerEncoder(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        ...
        self.layer = [TFLongformerLayer(
            config, i, name=f"layer_._{i}") for i in range(config.num_hidden_layers)]
...
class TFLongformerMainLayer(tf.keras.layers.Layer):
    ...
    def call(
        ...
        **kwargs,
        ...
        #     sequence_output =
        sequence_output = tf.cond(tf.math.greater(
            padding_len, 0), lambda: sequence_output[:, :-padding_len], lambda: sequence_output)
